  ...
  return (
    <label className="block">
      ...
      <div className="relative">
        ...
        />
        {suffix && <div className="absolute top-1/2 -translate-y-1/2 end-3 text-sm font-bold text-slate-500">{suffix}</div>}
      </div>
      {error && <span className={`mt-1 block text-xs font-semibold text-rose-600 ${isUr ? 'font-urdu u-text' : ''}`}>{error}</span>}
      {hint && !error && <span className={`mt-1 block text-xs text-slate-400 ${isUr ? 'font-urdu u-text' : ''}`}>{hint}</span>}
    </label>
  );
}
